...
import com.xplaza.backend.http.dto.OrderItemRequestDTO;
import com.xplaza.backend.http.dto.OrderItemResponseDTO;
...
public class OrderItemController extends BaseController {
  ...
  public ResponseEntity<String> getOrderItems() throws JsonProcessingException, JSONException {
    start = new Date();
    List<OrderItemEntity> entities = orderItemService.listOrderItems();
    List<OrderItemResponseDTO> dtos = entities.stream().map(orderItemMapper::toResponseDTO).toList();
    end = new Date();
  ...
  public ResponseEntity<String> getOrderItem(@PathVariable @Valid Long id) throws JsonProcessingException {
    start = new Date();
    OrderItemEntity entity = orderItemService.listOrderItem(id);
    OrderItemResponseDTO dto = orderItemMapper.toResponseDTO(entity);
    end = new Date();
    responseTime = end.getTime() - start.getTime();
    ObjectMapper mapper = new ObjectMapper();
    String response = "{\n" +
  ...
  @PostMapping
  public ResponseEntity<ApiResponse> addOrderItem(@RequestBody @Valid OrderItemRequestDTO orderItemRequestDTO) {
    start = new Date();
    OrderItemEntity entity = orderItemMapper.toEntity(orderItemRequestDTO);
    orderItemService.addOrderItem(entity);
    end = new Date();
    responseTime = end.getTime() - start.getTime();
