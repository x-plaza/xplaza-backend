...
import com.xplaza.backend.repository.ProductListRepository;
import com.xplaza.backend.repository.ProductRepository;
import com.xplaza.backend.repository.ProductSearchRepository;
...
public class ProductService {
  ...
  public List<ProductList> listProductsByShopIDByAdmin(Long shop_id) throws ParseException {
    ...
    listOfProducts.forEach(
        (i) -> i.setName(i.getName() + " (" + i.getProduct_var_type_value() + i.getProduct_var_type_name() + ")")); // Lambda
                                                                                                                    // function
    return listOfProducts;
  }
      ...
      if (p.getDiscount_amount() != null && p.getDiscount_type_name() != null) {
        ...
        if (current_date.compareTo(formatter.parse(p.getDiscount_start_date())) >= 0
          ...
          if (p.getDiscount_type_name().equals("Fixed Amount"))
            ...
            discounted_price = p.getSelling_price() - (p.getSelling_price() * p.getDiscount_amount()) / 100; // for
                                                                                                             // percentage
                                                                                                             // type
