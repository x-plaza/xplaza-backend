...
public interface LocationListRepository extends JpaRepository<LocationListDao, Long> {
  @Query(value = "select l.*, c.city_name from locations l left join cities c on l.fk_city_id = c.city_id", nativeQuery = true)
  List<LocationListDao> findAllItem();
  ...
  LocationListDao findLocationListById(Long id);
}
