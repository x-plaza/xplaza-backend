...
/**
 * Resolves a {@link UserDetails} from either the admin or customer table.
 * The role embedded in the JWT (claim {@code role}) tells us which lookup to
 * try first, but we fall back to the other repository so a customer JWT
 * cannot accidentally resolve to a stale admin entry and vice versa.
 */
@Primary
@Service
public class CompositeUserDetailsService implements UserDetailsService {
  ...
  /**
   * Like {@link #loadUserByUsername(String)} but biased by the role hint to
   * avoid an unnecessary first lookup against the wrong table.
   */
