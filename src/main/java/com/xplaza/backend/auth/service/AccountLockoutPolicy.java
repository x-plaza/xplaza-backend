 ...
 * - 5..7 failed attempts: 5-minute lockout
 * - 8+ failed attempts: 1-hour lockout (escalating geometric back-off)
 */
public final class AccountLockoutPolicy {
  ...
  public static Instant computeLockedUntil(int failedAttempts) {
    ...
    if (failedAttempts < 8) return Instant.now().plus(Duration.ofMinutes(5));
